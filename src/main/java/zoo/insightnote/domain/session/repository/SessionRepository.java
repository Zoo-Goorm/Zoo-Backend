...
// 1. 세션 전체 조회 (연사 이미지 제외, 인원수 제외, 키워드 포함)
    @Query("""
        SELECT DISTINCT s.id, s.name, k.name, s.shortDescription, s.location, s.startTime, s.endTime
        FROM Session s
    ...
    @Query("""
    SELECT DISTINCT s.id, s.name, s.shortDescription, s.maxCapacity, s.participantCount, 
